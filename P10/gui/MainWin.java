...
public class MainWin extends JFrame {
    ...
    public MainWin(String title) {
        ...
        JMenuItem iOrder = new JMenuItem("Orders");
        ...
        JButton insertOrder = new JButton(new ImageIcon("gui/resources/insertOrder.png"));
        insertOrder.setActionCommand("Insert Order");
        insertOrder.setToolTipText("Insert Order");
        insertOrder.setPreferredSize(new DimensionUIResource(64, 64));
        ...
        JButton viewOrder = new JButton(new ImageIcon("gui/resources/viewOrder.png"));
        viewOrder.setActionCommand("View Orders");
        viewOrder.setToolTipText("View Orders");
        viewOrder.setPreferredSize(new DimensionUIResource(64, 64));
    ...
    protected void onNewClick() {
        ...
        if (button == JOptionPane.OK_OPTION) {
            try {
                ...
                displayText.setText("<html><p><font size=+2> Welcome To Your New Store!</font></p></html>");
            } catch (Exception e) {
    ...
    protected void onOpenClick() {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(openFile))) {
                ...
                displayText.setText("<html><p><font size=+2> Welcome Back To " + this.getTitle() + "!</font></p></html>");
            } catch (Exception e) {
    ...
    protected void onInsertOrderClick() {
        Customer cust = null;
        Object[] customerArr = store.customers();
        if (customerArr.length == 0) {
            onInsertCustomerClick();
            customerArr = store.customers();
            if (customerArr.length == 0) {
                return;
            } else {
                cust = (Customer) customerArr[0];
            }
        } else if (customerArr.length == 1) {
            cust = (Customer) customerArr[0];
        } else if (customerArr.length >= 2) {
            JLabel lblCustomers = new JLabel("Order for which Customer?");
            JComboBox<Object> cboCustomers = new JComboBox<>(customerArr);

            // Display OptionPane
            Object[] objects = { lblCustomers, cboCustomers };

            int button = JOptionPane.showConfirmDialog(this, objects, "Select Customer For Order",
                    JOptionPane.OK_CANCEL_OPTION,
                    JOptionPane.PLAIN_MESSAGE);

            if (button == JOptionPane.OK_OPTION) {
                cust = (Customer) cboCustomers.getSelectedItem();
            } else {
                return;
            }
        }

        // Instance a new Order
        Order newOrder = new Order(cust);
        int doneOrdering = 100;

        while (doneOrdering != 0 && doneOrdering != 2 && doneOrdering != -1) {
            JTextArea txtCurrentOrder = new JTextArea(newOrder.toString());
            JScrollPane test = new JScrollPane(txtCurrentOrder); test.setPreferredSize(new Dimension(200, 200));
            JLabel lblAvailComps = new JLabel("<HTML><br/><p>Available Computers</p><HTML>");
            JComboBox<Object> cboComputers = new JComboBox<>(store.computers());
            JButton btnAddComp = new JButton("Add Computer"); 
            btnAddComp.addActionListener(event -> {newOrder.addComputer((Computer) cboComputers.getSelectedItem()); txtCurrentOrder.setText(newOrder.toString()); btnAddComp.setActionCommand("Computer Added");});
            JLabel lblDoneOrdering = new JLabel("<HTML><br/><p>If you are done ordering, press \"OK\" to complete the order.<br/></p></HTML>");
            
            // Display the OptionPane
            Object[] objects = { test, lblAvailComps, cboComputers, btnAddComp, lblDoneOrdering };
            
            doneOrdering = JOptionPane.showConfirmDialog(this, objects, "Insert Order", JOptionPane.OK_CANCEL_OPTION,
                    JOptionPane.PLAIN_MESSAGE);
    
            // Done with Order
            if (doneOrdering == JOptionPane.OK_OPTION) {
                try {
                    if(btnAddComp.getActionCommand() == "Computer Added") {store.add(newOrder);}
                    onViewClick(Record.ORDER);
                } catch (Exception e) {
                    JOptionPane.showMessageDialog(this, e.getMessage(), "Error With Inserting Order",
                            JOptionPane.ERROR_MESSAGE);
                }
            }             
        }
    ...
    protected void onViewClick(Record record) {
        ...
        displayText.setText("<html><p><font size=+2>" + lblHeader.getText() + "</font></p><br/><ol>");
        ...
        displayText.setText(displayText.getText() + lblData.getText() + "</ol></html>");
    }
    ...
    protected void onAboutClick() { // Display About dialog
        ...
        JLabel artists = new JLabel("<html>"
                ...
                + "<br/><p>Insert Order, licensed for personal and commercial purposes with attribution</p>"
                + "<p><font size=-2>https://www.flaticon.com/free-icon/add-cart_4153710 - Created by Andika Syaif</font></p>"
                ...
                + "<br/><p>View Orders, licensed for personal and commercial purposes with attribution</p>"
                + "<p><font size=-2>https://www.flaticon.com/free-icon/search_3007219 - Created by Smashicons</font></p>"
                + "</html>");
